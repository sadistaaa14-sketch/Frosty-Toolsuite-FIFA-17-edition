...
{
    public class DuplicateShoeMenuExtension : MenuExtension
    {
        ...
        public DuplicateShoeMenuExtension()
        {
        ...
        public override string MenuItemName => "Duplicate Shoe";
        ...
        public override RelayCommand MenuItemClicked => new RelayCommand((o) =>
        {
            ...
            if (entry == null)
            {
                FrostyMessageBox.Show(
                    "No asset selected. Select any asset inside the shoe folder you want to duplicate.",
                    "Shoe Duplicator");
                return;
            }
            ...
            {
                FrostyMessageBox.Show("Selected asset has no folder path.", "Shoe Duplicator");
                return;
            }

            if (sourceFolder.EndsWith("_launch_shoe_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_launch_shoe_brt".Length);
            else if (sourceFolder.EndsWith("_shoe_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_shoe_brt".Length);

            string sourceShoeName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string oldId = ExtractId(sourceShoeName);
            if (string.IsNullOrEmpty(oldId))
            {
                FrostyMessageBox.Show(
                    "Could not extract a numeric ID from folder name '" + sourceShoeName + "'.\n" +
                    "Expected format: shoe_999",
                    "Shoe Duplicator");
                return;
            }

            DuplicateShoeWindow win = new DuplicateShoeWindow(sourceFolder);
            if (win.ShowDialog() != true)
                return;

            string newName = win.NewShoeName;
            string destPath = win.DestinationPath;

            FrostyTaskWindow.Show("Duplicating Shoe", "", (task) =>
            {
                try
                {
                    ...
                    DuplicateShoe(task, sourceFolder, newName, destPath);
                }
                catch (Exception ex)
                {
                    App.Logger.Log("Error duplicating shoe: " + ex.ToString());
                }
            });
        ...
        {
            string sourceShoeName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string newFolder = destPath.TrimEnd('/') + "/" + newShoeName;

            string oldId = ExtractId(sourceShoeName);
            string newId = ExtractId(newShoeName);
            ...
            App.Logger.Log("Source: " + sourceShoeName + " (ID " + oldId + ")");
            App.Logger.Log("Target: " + newShoeName + " (ID " + newId + ")");

            // ── Phase 1: Enumerate ──────────────────────────────────────────────
            task.Update("Finding source assets...");

            string sourceBrtFolder1 = sourceFolder + "_shoe_brt";
            string sourceBrtFolder2 = sourceFolder + "_launch_shoe_brt";
            ...
            App.Logger.Log("Shoe duplication complete (" + allNew.Count + " assets)");
        }
