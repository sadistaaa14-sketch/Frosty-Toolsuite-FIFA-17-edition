...
{
    public class DuplicateGenericHeadMenuExtension : MenuExtension
    {
        ...
        public DuplicateGenericHeadMenuExtension()
        {
        ...
        public override string MenuItemName => "Duplicate Generic Head";
        ...
        public override RelayCommand MenuItemClicked => new RelayCommand((o) =>
        {
            ...
            if (entry == null)
            {
                FrostyMessageBox.Show(
                    "No asset selected. Select any asset inside the generic head folder you want to duplicate.",
                    "Generic Head Duplicator");
                return;
            }
            ...
            {
                FrostyMessageBox.Show("Selected asset has no folder path.", "Generic Head Duplicator");
                return;
            }
            ...
            if (string.IsNullOrEmpty(oldId))
            {
                FrostyMessageBox.Show(
                    ...
                    "Expected format: generic_caucasian_0",
                    "Generic Head Duplicator");
                return;
            }

            DuplicateGenericHeadWindow win = new DuplicateGenericHeadWindow(sourceFolder);
            if (win.ShowDialog() != true)
                return;

            string newName = win.NewHeadName;
            string destPath = win.DestinationPath;

            FrostyTaskWindow.Show("Duplicating Generic Head", "", (task) =>
            {
                try
                {
                    ...
                    DuplicateGenericHead(task, sourceFolder, newName, destPath);
                }
                catch (Exception ex)
                {
                    App.Logger.Log("Error duplicating generic head: " + ex.ToString());
                }
            });
        ...
        {
            string sourceName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string newFolder = destPath.TrimEnd('/') + "/" + newHeadName;

            string oldId = ExtractId(sourceName);
            string newId = ExtractId(newHeadName);
            ...
            App.Logger.Log("Target: " + newHeadName + " (ID " + newId + ")");
            ...
            App.Logger.Log("Generic head duplication complete (" + allNew.Count + " assets)");
        }
