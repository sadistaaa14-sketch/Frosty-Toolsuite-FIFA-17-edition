...
{
    public class DuplicateBallMenuExtension : MenuExtension
    {
        ...
        public DuplicateBallMenuExtension()
        {
        ...
        public override string MenuItemName => "Duplicate Ball";
        ...
        public override RelayCommand MenuItemClicked => new RelayCommand((o) =>
        {
            ...
            if (entry == null)
            {
                FrostyMessageBox.Show(
                    "No asset selected. Select any asset inside the ball folder you want to duplicate.",
                    "Ball Duplicator");
                return;
            }
            ...
            {
                FrostyMessageBox.Show("Selected asset has no folder path.", "Ball Duplicator");
                return;
            }

            if (sourceFolder.EndsWith("_launch_ball_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_launch_ball_brt".Length);
            else if (sourceFolder.EndsWith("_ball_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_ball_brt".Length);

            string sourceBallName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string oldId = ExtractId(sourceBallName);
            if (string.IsNullOrEmpty(oldId))
            {
                FrostyMessageBox.Show(
                    "Could not extract a numeric ID from folder name '" + sourceBallName + "'.\n" +
                    "Expected format: ball_999",
                    "Ball Duplicator");
                return;
            }

            DuplicateBallWindow win = new DuplicateBallWindow(sourceFolder);
            if (win.ShowDialog() != true)
                return;

            string newName = win.NewBallName;
            string destPath = win.DestinationPath;

            FrostyTaskWindow.Show("Duplicating Ball", "", (task) =>
            {
                try
                {
                    ...
                    DuplicateBall(task, sourceFolder, newName, destPath);
                }
                catch (Exception ex)
                {
                    App.Logger.Log("Error duplicating ball: " + ex.ToString());
                }
            });
        ...
        internal void DuplicateBall(FrostyTaskWindow task, string sourceFolder,
            string newBallName, string destPath)
        {
            string sourceBallName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string newFolder = destPath.TrimEnd('/') + "/" + newBallName;

            string oldId = ExtractId(sourceBallName);
            string newId = ExtractId(newBallName);
            ...
            App.Logger.Log("Source: " + sourceBallName + " (ID " + oldId + ")");
            App.Logger.Log("Target: " + newBallName + " (ID " + newId + ")");

            // ── Phase 1: Enumerate ──────────────────────────────────────────────
            task.Update("Finding source assets...");

            string sourceBrtFolder1 = sourceFolder + "_ball_brt";
            string sourceBrtFolder2 = sourceFolder + "_launch_ball_brt";
            ...
            App.Logger.Log("Ball duplication complete (" + allNew.Count + " assets)");
        }
