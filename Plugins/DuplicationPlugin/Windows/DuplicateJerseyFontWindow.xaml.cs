...
{
    public partial class DuplicateJerseyFontWindow : FrostyDockableWindow
    {
        public string NewFolderName { get; private set; }

        private readonly string sourceFolderName;

        public DuplicateJerseyFontWindow(string inSourceFolder)
        {
            InitializeComponent();
        ...
        private void DuplicateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                FrostyMessageBox.Show("New folder name cannot be empty.", "Jersey Font Duplicator");
                return;
            }

            if (newFolder.Contains("/") || newFolder.Contains(" "))
            {
                FrostyMessageBox.Show("Folder name contains invalid characters.", "Jersey Font Duplicator");
                return;
            }
            ...
            if (lastUnderscore < 0)
            {
                FrostyMessageBox.Show(
                    "New folder name must end with a numeric id.\nExample: acmilan_99",
            ...
            {
                FrostyMessageBox.Show("New folder name must be different from the source.", "Jersey Font Duplicator");
                return;
            }
