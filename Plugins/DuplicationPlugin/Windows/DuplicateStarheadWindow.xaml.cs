...
{
    public partial class DuplicateStarheadWindow : FrostyDockableWindow
    {
        ...
        public DuplicateStarheadWindow(string inSourceFolder)
        {
            InitializeComponent();
            ...
            string sourcePlayerName = inSourceFolder.Substring(inSourceFolder.LastIndexOf('/') + 1);
            newNameTextBox.Text = sourcePlayerName;
        ...
        private void DuplicateButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                FrostyMessageBox.Show("New player name cannot be empty.", "Starhead Duplicator");
                return;
            }
 
            if (newName.Contains("//") || newName.Contains(" "))
            {
                FrostyMessageBox.Show("Name contains invalid characters.", "Starhead Duplicator");
                return;
            }
            ...
            if (lastUnderscore < 0)
            {
                FrostyMessageBox.Show(
                    "New player name must end with a numeric ID.\nExample: achraf_hakimi_235212",
                    "Starhead Duplicator");
                return;
            }
            ...
            if (newName.Equals(sourcePlayerName, StringComparison.OrdinalIgnoreCase))
            {
                FrostyMessageBox.Show("New name must be different from the source.", "Starhead Duplicator");
                return;
            }
            ...
            {
                FrostyMessageBox.Show("Select a destination folder in the tree.", "Starhead Duplicator");
                return;
            }
