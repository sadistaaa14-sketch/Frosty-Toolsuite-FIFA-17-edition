...
{
    public class DuplicateLeagueLogoMenuExtension : MenuExtension
    {
        ...
        public DuplicateLeagueLogoMenuExtension()
        {
        ...
        public override string MenuItemName => "Duplicate League Logo";
        ...
        public override RelayCommand MenuItemClicked => new RelayCommand((o) =>
        {
            ...
            if (entry == null)
            {
                FrostyMessageBox.Show(
                    "No asset selected. Select any asset inside the league logo folder you want to duplicate.",
                    "League Logo Duplicator");
                return;
            }
            ...
            {
                FrostyMessageBox.Show("Selected asset has no folder path.", "League Logo Duplicator");
                return;
            }

            if (sourceFolder.EndsWith("_launch_league_logo_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_launch_league_logo_brt".Length);
            else if (sourceFolder.EndsWith("_leaguelogo_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_leaguelogo_brt".Length);
            ...
            if (string.IsNullOrEmpty(oldId))
            {
                FrostyMessageBox.Show(
                    ...
                    "Expected format: australia_351",
                    "League Logo Duplicator");
                return;
            }

            DuplicateLeagueLogoWindow win = new DuplicateLeagueLogoWindow(sourceFolder);
            if (win.ShowDialog() != true)
                return;

            string newName = win.NewLogoName;
            string destPath = win.DestinationPath;

            FrostyTaskWindow.Show("Duplicating League Logo", "", (task) =>
            {
                try
                {
                    ...
                    DuplicateLeagueLogo(task, sourceFolder, newName, destPath);
                }
                catch (Exception ex)
                {
                    App.Logger.Log("Error duplicating league logo: " + ex.ToString());
                }
            });
        ...
        {
            string sourceName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string newFolder = destPath.TrimEnd('/') + "/" + newLogoName;

            string oldId = ExtractId(sourceName);
            string newId = ExtractId(newLogoName);
            ...
            App.Logger.Log("Target: " + newLogoName + " (ID " + newId + ")");

            // ── Phase 1: Enumerate ──────────────────────────────────────────────
            task.Update("Finding source assets...");

            string sourceBrtFolder1 = sourceFolder + "_leaguelogo_brt";
            string sourceBrtFolder2 = sourceFolder + "_launch_league_logo_brt";
            ...
            App.Logger.Log("League logo duplication complete (" + allNew.Count + " assets)");
        }
