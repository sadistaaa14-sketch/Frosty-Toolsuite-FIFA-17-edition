...
{
    public class DuplicateManagerMenuExtension : MenuExtension
    {
        ...
        public DuplicateManagerMenuExtension()
        {
        ...
        public override string MenuItemName => "Duplicate Manager";
        ...
        public override RelayCommand MenuItemClicked => new RelayCommand((o) =>
        {
            ...
            if (entry == null)
            {
                FrostyMessageBox.Show(
                    "No asset selected. Select any asset inside the manager folder you want to duplicate.",
                    "Manager Duplicator");
                return;
            }
            ...
            {
                FrostyMessageBox.Show("Selected asset has no folder path.", "Manager Duplicator");
                return;
            }

            if (sourceFolder.EndsWith("_launch_manager_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_launch_manager_brt".Length);
            else if (sourceFolder.EndsWith("_manager_brt", StringComparison.OrdinalIgnoreCase))
                sourceFolder = sourceFolder.Substring(0, sourceFolder.Length - "_manager_brt".Length);

            string sourceManagerName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string oldId = ExtractId(sourceManagerName);
            if (string.IsNullOrEmpty(oldId))
            {
                FrostyMessageBox.Show(
                    "Could not extract a numeric manager ID from folder name '" + sourceManagerName + "'.\n" +
                    "Expected format: antonio_conte_1067",
                    "Manager Duplicator");
                return;
            }

            DuplicateManagerWindow win = new DuplicateManagerWindow(sourceFolder);
            if (win.ShowDialog() != true)
                return;

            string newManagerName = win.NewManagerName;
            string destPath = win.DestinationPath;

            FrostyTaskWindow.Show("Duplicating Manager", "", (task) =>
            {
                try
                {
                    ...
                    DuplicateManager(task, sourceFolder, newManagerName, destPath);
                }
                catch (Exception ex)
                {
        ...
        {
            string sourceManagerName = sourceFolder.Substring(sourceFolder.LastIndexOf('/') + 1);
            string newFolder = destPath.TrimEnd('/') + "/" + newManagerName;

            string oldId = ExtractId(sourceManagerName);
            string newId = ExtractId(newManagerName);

            if (string.IsNullOrEmpty(oldId) || string.IsNullOrEmpty(newId))
            {
                App.Logger.Log("Could not extract manager IDs. Aborting.");
                return;
            }

            App.Logger.Log("Source: " + sourceManagerName + " (ID " + oldId + ")");
            App.Logger.Log("Target: " + newManagerName + " (ID " + newId + ")");

            // ── Phase 1: Enumerate ──────────────────────────────────────────────
            task.Update("Finding source assets...");

            // Check both possible BRT subfolder suffixes
            string sourceBrtFolder1 = sourceFolder + "_manager_brt";
            string sourceBrtFolder2 = sourceFolder + "_launch_manager_brt";
